...
from resume_scraper.resume_praser import parse_resume_from_file, generate_resume_summary
...
# app.config['SECRET_KEY'] = os.getenv('FLASK_SECRET_KEY', 'mmmmm33445666nnn_default_secret') # OLD LINE
app.config['SECRET_KEY'] = os.getenv('FLASK_SECRET_KEY') # <--- UPDATED LINE - now it will strictly come from .env or env var
...
class ResumeJobMatcher:
    ...
    def scrape_job_listings(self, cities: List[str], primary_job_title: Optional[str] = None) -> List[Dict]:
        """
        ...
        # Determine keywords for search based on primary job title or broad categories
        search_keywords = ["software", "developer", "engineer", "data scientist", "analyst", "manager", "designer"]
        if primary_job_title:
            # Use the inferred job title from the resume as a primary keyword
            search_keywords = [primary_job_title] + list(set(search_keywords) - {primary_job_title.lower()}) # Add to front, remove if duplicate

        for city in cities:
            encoded_city = urllib.parse.quote_plus(f"{city}, Nepal")
            # Create search URLs for common roles within the city
...
"""
    ...
    def match_resume_to_jobs(self, resume_data: Dict, job_listings: List[Dict]) -> List[Dict]:
        ...
        primary_job_title = resume_data.get("Work Experience", [{}])[0].get("Position") \
                           or resume_data.get("Projects", [{}])[0].get("Name") \
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                # Determine primary job title from resume for better search queries
                primary_job_title = resume_data.get("Work Experience", [{}])[0].get("Position") or \
                                    resume_data.get("Projects", [{}])[0].get("Name")

                nepal_cities = ["Kathmandu", "Pokhara", "Lalitpur"] # Major cities in Nepal
                job_listings = matcher.scrape_job_listings(nepal_cities, primary_job_title)

                if not job_listings:
                    flash('No job listings found for the specified cities/keywords. Please try again later or adjust your resume content.', 'warning')
...
def download_results():
    # It's better to pass a unique ID or use session to retrieve complex data
    # rather than sending large JSON via query params.
    # For simplicity, keeping original approach but noting the limitation.
...
if __name__ == '__main__':
    # It is highly recommended to set FLASK_SECRET_KEY as an environment variable
    # For example: export FLASK_SECRET_KEY='your_very_secret_key_here'
    app.run(debug=True)
# --- END OF FILE cli4.py ---c