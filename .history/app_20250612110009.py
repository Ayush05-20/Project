...
from flask import Flask, render_template, request, flash, redirect, url_for, send_file, session # ADDED session
...
# For PDF generation
from weasyprint import HTML # ADDED WeasyPrint import
...
class ResumeJobMatcher:
    ...
    def scrape_job_listings(self, cities: List[str], primary_job_title: Optional[str] = None) -> List[Dict]:
        ...
        if primary_job_title:
            search_keywords.append(primary_job_title)
        # Add general fallback keywords if primary is not specific enough or missing
        if not primary_job_title or "inferred" in primary_job_title.lower():
            search_keywords.extend(["software engineer", "developer", "data scientist", "it support", "marketing specialist", "graphic designer", "business analyst"])
